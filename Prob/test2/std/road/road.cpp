...
int n, m, k, ans, dis1[maxn], dis2[maxn], use[21][210][21];
bool vis[maxn];
...
inline void DFS(int u, int sum, int cost, int num)
{
    if ( (cost + dis1[u] > k) || (sum + dis2[u] >= ans) ) return ;
    if ( u == n ) { if ( cost == k ) ans = sum; return ; } 
    int x = rand() % 5;
    if ( use[u][cost][num] < sum && x != 0 && x != 1 ) return ;
    use[u][cost][num] = min(use[u][cost][num], sum);
    for ( int i = Begin[u]; i; i = Next[i] ) 
    {
        int v = To[i]; if ( vis[v] ) continue ;
        vis[v] = true;
        DFS(v, sum + W[i], cost + C[i], num + 1);
        vis[v] = false;
    }
}
