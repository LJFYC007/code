/***************************************************************
	File name: P3355.cpp
	Author: ljfcnyali
	Create time: 2019年08月09日 星期五 21时13分19秒
***************************************************************/
#include<bits/stdc++.h>
...
int n, m, s, t, a[210][210], dt[8][2] = {{1, 2}, {2, 1}, {-1, 2}, {2, -1}, {1, -2}, {-2, 1}, {-1, -2}, {-2, -1}};
...
int main()
{
    ...
    REP(i, 1, m) { int x, y; scanf("%d%d", &x, &y); a[x][y] = 0; } 
    REP(i, 1, n) REP(j, 1, n) 
    {
        if ( !a[i][j] ) continue ;
        int x = (i - 1) * n + j;
        if ( (i + j) % 2 == 0 ) { Dinic :: add(x, t, 1); continue ; }
        Dinic :: add(s, x, 1);
        REP(k, 0, 7)
        {
