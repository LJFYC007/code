/***************************************************************
	File name: P2763.cpp
	Author: ljfcnyali
	Create time: 2019年07月20日 星期六 16时14分25秒
***************************************************************/
#include<bits/stdc++.h>
...
int main()
{
    ...
    scanf("%d%d", &m, &n);
    t = n + m + 1;
    REP(i, 1, m) { int x; scanf("%d", &x); Dinic :: add(i, t, x); sum += x; } 
    REP(i, 1, n)
    {
        int k; scanf("%d", &k);
        Dinic :: add(s, m + i, 1);
        REP(j, 1, k) { int x; scanf("%d", &x); Dinic :: add(m + i, x, 1); }
    }
    if ( Dinic :: Solve(s, t) != sum ) { printf("No Solution!\n"); return 0; }
    REP(i, 1, m)
    {
        printf("%d:", i);
        Dinic :: DFS(i);
