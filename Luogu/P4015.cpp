/***************************************************************
	File name: P4015.cpp
	Author: ljfcnyali
	Create time: 2019年08月16日 星期五 09时53分50秒
***************************************************************/
#include<bits/stdc++.h>
...
int n, m, s, t, a[maxn], b[maxn], c[110][110];
...
int main()
{
    ...
    scanf("%d%d", &n, &m);
    t = n + m + 1;
    REP(i, 1, n) { scanf("%d", &a[i]); MCMF :: add(s, i, a[i], 0); }
    REP(i, 1, m) { scanf("%d", &b[i]); MCMF :: add(i + n, t, b[i], 0); }
    REP(i, 1, n) REP(j, 1, m) { scanf("%d", &c[i][j]); MCMF :: add(i, n + j, INF, c[i][j]); }
    int flow, cost;
    MCMF :: Solve(s, t, flow, cost);
    printf("%d\n", cost);
    MCMF :: INIT();
    REP(i, 1, n) MCMF :: add(s, i, a[i], 0);
    REP(i, 1, m) MCMF :: add(i + n, t, b[i], 0);
    REP(i, 1, n) REP(j, 1, m) MCMF :: add(i, n + j, INF, -c[i][j]);
    MCMF :: Solve(s, t, flow, cost);
    printf("%d\n", -cost);
    return 0;
}
