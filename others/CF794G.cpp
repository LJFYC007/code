...
int n, miu[maxn], prime[maxn], tot, fac[maxn], inv[maxn];
bool vis[maxn];
char s[2][maxn];
...
inline int calc(int n) { return power(2, n + 1) - 2; }
...
inline int Solve(int a, int b)
{
    if ( a == b && a == 0 )
    {
        ...
        {
            int x = power(2, d) * miu[t / d] * (n / t) % Mod;
            ret = (ret + x * (n / t)) % Mod;
        }
        return ret;
    }
...
signed main()
{
    ...
    {
        REP(i, 1, str(s[0] + 1))
            if ( s[0][i] != '?' && s[1][i] != '?' && s[0][i] != s[1][i] ) goto Next ;
        int x = 0;
        REP(i, 1, str(s[0] + 1)) if ( s[0][i] == '?' && s[1][i] == '?' ) ++ x;
