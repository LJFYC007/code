...
inline int LCA(int x, int y)
{
    ...
    for ( int i = 10; i >= 0; -- i ) while ( dis[anc[x][i]] >= dis[y] ) x = anc[x][i];
    if ( x == y ) return x;
    for ( int i = 10; i >= 0; -- i ) while ( anc[x][i] != anc[y][i] ) { x = anc[x][i]; y = anc[y][i]; }
    return anc[x][0];
}
...
{
    S.reset();
    for ( int i = 10; i >= 0; -- i ) while ( dis[anc[s][i]] >= dis[t] ) 
    {
        S |= f[s][i]; s = anc[s][i];
    }
    return S;
