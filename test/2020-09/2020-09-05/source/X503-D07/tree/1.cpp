...
int type, n, m, k, q, M, num[maxn], fa[maxn], ans, c[maxn], fac[maxn], inv[maxn], size[maxn];
int anc[maxn][20], dis[maxn], val1[maxn][11], val2[maxn][11], val3[maxn][11], s[maxn];
bool leaf[maxn];
vector<int> f1[maxn][11], f2[maxn][11], Edge[maxn];
...
{ 
    int ret = 0, sum = 1; 
    REP(i, 0, 10) { ret = (ret + a[i] * sum) % Mod; sum = sum * x % Mod; }
    return ret; 
}
...
{
    if ( !w ) return f;
    vector<int> g; g.resize(11);
    int p[11]; p[0] = 1; REP(i, 1, 10) p[i] = p[i - 1] * w % Mod;
    REP(i, 0, 10) REP(j, 0, i) g[j] = (g[j] + (f[i] * C(i, j) % Mod) * p[i - j]) % Mod;
    return g;
}
...
{
    size[u] = 0; 
    for ( auto v : Edge[u] ) { DFS(v, l, r); size[u] += size[v]; }
    if ( l <= u && u <= r && !Edge[u].size() ) size[u] = 1; 
    if ( u != 1 ) REP(i, 0, k) ans = (ans + power(size[u], i) * c[i]) % Mod;
}

inline void DFS1(int u) 
{ 
    int sum = 0; REP(i, 0, 10) f1[u][i].resize(11);
...
{ 
    int sum = 0; REP(i, 0, 10) { val3[u][i] = 0; f2[u][i].resize(11); }
...
inline void Solve2(int l, int r, int x, int y)
{
    REP(i, 0, k)
    {
        ...
        ans = (ans + c[i] * (val3[r][i] - power(size[r], i))) % Mod;
        ans = (ans + c[i] * (Get(f2[r][i], y) - Get(f2[l][i], x))) % Mod;
    }
}
...
signed main()
{
    ...
    REP(i, 2, n) { scanf("%lld", &fa[i]); Edge[fa[i]].push_back(i); leaf[fa[i]] = 0; }
    REP(i, 1, n) num[i] = leaf[i] + num[i - 1];
    dis[1] = 1; DFS1(1); DFS2(1); REP(j, 1, 19) REP(i, 1, n) anc[i][j] = anc[anc[i][j - 1]][j - 1];
    ...
    while ( q -- ) 
    {
        ...
        REP(i, 0, k) c[k - i] = (c[k - i] + sgn(k - i) * C(k, i) * power(m, i)) % Mod;
        ...
        else 
        {
            ...
            Solve1(y, r, numy); Solve2(x, l, numx, num[r + s[r] - 1] - num[r - 1]);
            REP(i, 0, k) ans = (ans + c[i] * (power(numx, i) + power(numy, i))) % Mod;
            for ( auto v : Edge[Lca] ) if ( v != x && v != y && l <= v && v <= r ) 
                REP(i, 0, k) ans = (ans + c[i] * val3[v][i]) % Mod;
