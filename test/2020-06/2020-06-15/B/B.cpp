...
pii anc[51][17][maxn];
struct node { pii f[51]; int lazy; } Tree[maxn << 3];
...
inline void PushTag(int root, int l, int r, int x)
{
    Tree[root].lazy = x;
    int p = 1, num = 0;
    while ( p < (r - l + 1) ) { p <<= 1; ++ num; }
    REP(i, 0, N)
    {
        int posl = l - g[x].pos + 1;
        Tree[root].f[i] = anc[i][num][g[x].l + (posl - 1) % g[x].len];
    }
}
...
int main()
{
    ...
    BFS(); scanf("%s", s + 1); n = str(s + 1); 
    int NN = 1; while ( NN < n ) NN <<= 1;
    REP(i, n + 1, NN) s[i] = 'a'; Build(1, 1, NN);
    int o = 0;
    while ( q -- ) 
    {
        ...
        if ( op == 1 ) 
        {
            ...
            REP(j, 0, N) REP(k, 1, m) 
                anc[j][0][g[o].l + k - 1] = pii(Trie[j].Next[s[k] - 'a'], Trie[Trie[j].Next[s[k] - 'a']].val);
