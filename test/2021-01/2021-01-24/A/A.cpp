...
int n, m, a[maxn], p[1 << 15], n1, n2, f[maxn];
LL q[1 << 15], h[1 << 15], b[maxn];
struct node { int x; LL w; } f1[maxm], f2[maxm], f3[maxm / 3], f4[maxm / 3], f5[maxm / 3];
...
{
    if ( x == n - m + 1 ) { n1 = 1; f1[n1] = {0, 0}; return ; } 
    DFS1(x + 1, f1, n1);
    REP(i, 1, n1) 
    { 
        f3[i] = f1[i];
        f4[i] = {f1[i].x + f[x - 1], f1[i].w + a[m + x] * 32ll + 1};
        f5[i] = {f1[i].x + f[x - 1] + f[x - 1], f1[i].w - a[m + x] * 32ll - 1};
    }
    int now1 = 1, now2 = 1, now3 = 1, N = 0; 
    while ( now1 <= n1 || now2 <= n1 || now3 <= n1 ) 
    {
        node t = {0, INF}; int pos = 0;
...
{
    if ( x == m + 1 ) { n1 = 1; f1[n1] = {0, 0}; return ; } 
    DFS2(x + 1, f1, n1);
    REP(i, 1, n1) 
    { 
        f3[i] = f1[i];
        f4[i] = {f1[i].x + f[x - 1], f1[i].w + a[x] * 32ll + 1};
        f5[i] = {f1[i].x + f[x - 1] + f[x - 1], f1[i].w - a[x] * 32ll - 1};
    }
    int now1 = 1, now2 = 1, now3 = 1, N = 0; 
    while ( now1 <= n1 || now2 <= n1 || now3 <= n1 ) 
    {
...
signed main()
{
    ...
    REP(now2, 1, n2)
    {
        while ( now1 > 0 && f1[now1].w > -f2[now2].w ) -- now1;
        if ( now1 > 0 && f2[now2].x == 0 ) -- now1;
        if ( now1 && f1[now1].w == -f2[now2].w ) 
        {
            // cerr << now1 << " " << now2 << " " << f1[now1].x << " " << f2[now2].x << endl;
            int S = 0, s = 0, t = 0; LL x = 0;
            REP(i, 1, m) 
            {
                if ( (f2[now2].x / f[i - 1]) % 3 == 1 ) { S |= (1 << i - 1); s |= (1 << i - 1); }
                if ( (f2[now2].x / f[i - 1]) % 3 == 2 ) { S |= (1 << i - 1); t |= (1 << i - 1); }
            }
            REP(i, 1, n - m) 
            {
                if ( (f1[now1].x / f[i - 1]) % 3 == 1 ) { S |= (1 << m + i - 1); s |= (1 << m + i - 1); }
                if ( (f1[now1].x / f[i - 1]) % 3 == 2 ) { S |= (1 << m + i - 1); t |= (1 << m + i - 1); }
            }
            int l = get(s, 0), r = get(t, 0);
            REP(o, 1, n)
            {
