...
struct node { int lson, rson, num[2], lazy, sum; } Tree[maxn << 6];
...
{ 
	++ tot; Tree[tot].num[0] = r - l + 1; Tree[tot].sum = 0;
	if ( l > 1 ) Tree[tot].val[0] = power(2, l - 1); 
	Tree[tot].val[1] = power(2, r);
	return tot; 
}

inline void PushUp(int root)
{
	REP(i, 0, 1) Tree[root].num[i] = (Tree[ls(root)].num[i] + Tree[rs(root)].num[i]) % Mod;
...
inline int Solve()
{
	int ans = Tree[1].sum, pos = find(Root, 0, N, 0, N, 1);
	ans = (ans - 1ll * (pos + 1) * power(2, pos + 1)) % Mod;
	return ans;
}

int main()
{
    freopen("fft9.in", "r", stdin);
	...
	while ( m -- ) 
	{
		...
		if ( pos != x ) Modify(Root, 0, N, x, pos - 1, op);
