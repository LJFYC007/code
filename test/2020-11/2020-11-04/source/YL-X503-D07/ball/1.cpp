...
inline void DFS(int u)
{
    ...
    for ( int i = Begin[u]; i; i = Next[i] ) 
    {
        ...
        REP(k, 0, size[v]) REP(j, 0, size[u])
            f[u][j + k] = (f[u][j + k] + h[j] * f[v][k]) % Mod;
        ++ son; size[u] += size[v];
    }
    int x = f[u][0];
    REP(j, 0, size[u] - 1) f[u][j] = (f[u][j + 1] * (j + 1) % Mod) * power(size[u] - j, Mod - 2) % Mod;
    f[u][0] += x;
    if ( !son ) { REP(i, 0, size[u]) f[u][i] = 0; f[u][a[u] - 1] = inv[a[u] - 1]; }
}
