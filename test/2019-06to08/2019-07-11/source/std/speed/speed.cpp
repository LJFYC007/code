...
struct E
{
    int t,x,y;
    E(){}
    E(int _t,int _x,int _y){t=_t,x=_x,y=_y;}
}q[N<<2];
...
void dfs(int x){
  ...
  for(int i=g[x];i;i=nxt[i])if(v[i]!=f[x]){
    ...
    if(size[v[i]]>size[son[x]])son[x]=v[i]; // 记录重儿子
  }
}
...
void ins(int x,int a,int b,int c,int d,int p,int q){
    // x为root, a, b即线段树区间，c，d为l，r区间，p, q即uv
  if(c<=a&&b<=d){
    V[++ED]=p; // To[++ e] = u; W[e] = v; Net[e] = Begin[x]; Begin[x] = e;:e 
    W[ED]=q; 
    NXT[ED]=G[x];
    G[x]=ED;
    return;
