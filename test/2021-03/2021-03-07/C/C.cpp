...
int n, m, k, q, Begin[maxn], Next[maxn], To[maxn], e = 1, id[61][61];
int dis[maxn], anc[maxn][12];
char s[61][61], t[maxn][27];
map<string, int> Map;
struct Matrix { int n, m, a[45][45]; } pre[maxn][12], suf[maxn][12], now;
...
inline int LCA(int x, int y)
{
	...
	for ( int i = 11; i >= 0; -- i ) if ( dis[anc[x][i]] >= dis[y] ) x = anc[x][i];
	cerr << x << " " << y << endl;
	if ( x == y ) return x;
	for ( int i = 11; i >= 0; -- i ) if ( anc[x][i] != anc[y][i] ) { x = anc[x][i]; y = anc[y][i]; } 
	return anc[x][0];
}
...
signed main()
{
	...
	while ( q -- ) 
	{
		...
		for ( int i = 11; i >= 0; -- i ) if ( dis[anc[u][i]] >= dis[t] ) { now = now * pre[u][i]; u = anc[u][i]; } 
		vector<pii> p;
		for ( int i = 11; i >= 0; -- i ) if ( dis[anc[v][i]] >= dis[t] ) { p.push_back(pii(v, i)); v = anc[v][i]; } 
