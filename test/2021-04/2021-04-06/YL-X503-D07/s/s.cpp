...
signed main()
{
	...
	REP(i, 1, m) 
	{
		...
		/*
		if ( x == find(1) && fuck(y, a[1] - W) ) { printf("%lld\n", i); return 0; }
		if ( x == find(n) && fuck(y, a[n] - W) ) { printf("%lld\n", i); return 0; }
		if ( y == find(1) && fuck(x, a[1] + W) ) { printf("%lld\n", i); return 0; }
		if ( y == find(n) && fuck(x, a[n] + W) ) { printf("%lld\n", i); return 0; }
		*/
		...
		if ( val && ans.upper_bound(val) != ans.end() ) { printf("%lld\n", i); return 0; } 

		if ( pos == find(1) && (-- Set[pos].upper_bound(pii(a[1], n + 1))) != Set[pos].begin() ) { printf("%lld\n", i); return 0; } 
		if ( pos == find(n) && Set[pos].lower_bound(pii(a[n], 0)) != (-- Set[pos].end()) ) { printf("%lld\n", i); return 0; } 
	}
	printf("%lld\n", -val);
    return 0;
}
