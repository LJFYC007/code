...
int dep[maxn], anc[maxn][18], belong[maxn], a[maxn], cnt[maxn], num, b[670][maxn / 2], len[maxn];
...
{
	if ( cnt[x] ) { REP(i, p[x].first, p[x].second) if ( a[i] ) a[i] = Get(a[i], cnt[x]); }
...
inline void Move(int x)
{
    ...
    {
        b[x][i] = f[b[x][i]]; const int t = b[x][i];
        if ( vis[x][t] ) 
        { 
            if ( i < len[x] ) { b[x][i] = b[x][len[x]]; -- i; }
            -- len[x];
        }
        else { vis[x][t] = true; Min[x] = min(Min[x], dis[t]); }
    }
}
...
signed main()
{
    ...
    dep[root] = 1; DFS(root, 0); REP(j, 1, 17) REP(i, 1, n) anc[i][j] = anc[anc[i][j - 1]][j - 1];
    int B = 400;
    for ( int l = 1, r = B; l <= n; l += B, r = min(n, r + B) )  { p[++ k] = pii(l, r); REP(i, l, r) belong[i] = k; }
    REP(i, 1, n) read(a[i]); REP(i, 0, 17) ccc[1 << i] = i; 
    ...
    while ( m -- ) 
    {
        ...
        else
        {
            ...
            {
				int t = cnt[belong[l]];
				if ( belong[l] == belong[r] ) REP(i, l, r) ans = min(ans, dis[Get(a[i], t)]);
				else 
				{
					int t = cnt[belong[l]]; REP(i, l, p[belong[l]].second) ans = min(ans, dis[Get(a[i], t)]);
					t = cnt[belong[r]]; REP(i, p[belong[r]].first, r) ans = min(ans, dis[Get(a[i], t)]);
				}
            }
            else
            {
