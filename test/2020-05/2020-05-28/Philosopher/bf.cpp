...
inline int power(__int128 a, int b)
{
    ...
    while ( b ) 
    {
        if ( b & 1 ) { r *= a; while ( r > N ) r /= 10; }
        a *= a; b >>= 1; while ( a > N ) a /= 10;
    }
    return r;
}
...
inline void PushUp(int root)
{
    ...
    while ( Tree[root].sum > N ) Tree[root].sum /= 10;
}
...
{
    // cerr << root << " " << l << " " << r  << " " << k << " " << Tree[root].size << endl;
...
{
    if ( !root ) return ;
    printf("%lld %lld %lld %lld %lld\n", root, l, r, Tree[root].size, (int) Tree[root].sum);
    int Mid = l + r >> 1;
    PRINT(ls(root), l, Mid);
...
signed main()
{
    ...
    // for ( auto i : Set ) printf("%lld ", i) ;puts("");
    // for ( auto i : Set ) printf("%lld ", (int)Tree[Root[i]].sum) ;puts("");
    return 0;
}
