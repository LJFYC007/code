...
int n, m, k, f[maxn][5][5][31], g[maxn][5][5][31][2], a[31], b[31], c[31];
...
{
    int d[maxn]; REP(i, 0, k) d[i] = 0;
    REP(i, 0, k) REP(j, 0, k) if ( i + j <= k ) add(d[i + j], a[i] * b[j]);
    REP(i, 0, k) a[i] = d[i];
}
    ...
    REP(i, 1, m) REP(x, 0, 4) REP(y, 0, 4) REP(j, 0, k) REP(o, 0, 1)
    {
        ...
        if ( o == 0 ) 
        {
            ...
            if ( x >= 2 && y >= 2 ) 
            {
                int val1 = C(4, x - 2) * inv(C(4, x)) % Mod;
                int val2 = C(4, y - 2) * inv(C(4, y)) % Mod;
                add(g[i][y - 2][0][j + 2][1], (g[i - 1][x][y][j][0] * val1 % Mod) * val2 % Mod);
            }
        }
    }

    REP(i, 0, k) REP(x, 0, 4) REP(y, 0, 4) a[i] = (a[i] + f[m][x][y][i]) % Mod;
    REP(i, 0, k) REP(x, 0, 4) REP(y, 0, 4) b[i] = (b[i] + g[m][x][y][i][1]) % Mod;

    if ( n == 1 ) { printf("%lld\n", (b[k] + Mod) % Mod); return 0; }

    REP(i, 0, k) c[i] = a[i];
    REP(i, 2, n - 1) Mul(c, a); Mul(c, b);
