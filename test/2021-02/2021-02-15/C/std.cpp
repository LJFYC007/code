...
inline Z &operator*=(Z &x1, const Z x2) { return x1 = x1 * x2; }
inline Z &operator/=(Z &x1, const Z x2) { return x1 = x1 / x2; }
Z fpm(Z a,int b){Z c(1);for(;b;b>>=1,a*=a)if(b&1)c*=a;return c;}
...
int n,k;
Z L[200005];
Z fac[200005],inv[200005],invf[200005];
Z f[200005];
Z ex[200005];
ll a[200005];
Z w;
namespace k1{
	...
	bool main(){
		...
		for(int i=1;i<=n;i+=2)f[i]=w*L[i];
		exp(n+1,f,g);
		Z ans(0);
		for(int i=1;i<=n;++i)ans+=g[i]*ex[n-i];
		ans/=w;
		ans+=ex[n];
		//ans*=fac[n];
		write(ans.x,'\n');
...
namespace run{
	...
	Z w[5005],xl[5005];
	Z s[5005];
	Z f[5005],g[5005],h[5005];
	bool main(){
		/*dp[1][0]=1;
		for(int i=1;i<n;++i)s[i]=fpm(2,n-i-1);
		s[n]=1;*/
		for(int xn=1;xn<=n;++xn){
			for(int i=1;i<=k+1;++i)for(int x=0;x<=xn;++x)dp[i][x]=0;	
			dp[1][0]=1;
			for(int i=0;i<xn;++i)s[i]=fpm(2,xn-i-1);
			s[xn]=1;
			for(int i=1;i<=k;++i){
				for(int x=0;x<=xn;++x)/*if(dp[i][x].x)*/{
					if(x!=xn)dp[i][x]*=fpm(s[x],(a[i]-a[i-1]-1)%(p-1));
					for(int y=x;y<=xn;++y){
						if(y!=xn || !(x&1))dp[i+1][y]+=dp[i][x]*s[y]*fac[y]*invf[x]*invf[y-x];
					}
				}
			}
			w[xn]=dp[k+1][xn]*invf[xn];
		}
		f[0]=1;
		g[0]=1;
		for(int i=1;i<=n;i+=2)xl[i]=L[i];
		for(int i=1;i<=n;++i){
			for(int x=0;x<=n;++x)for(int y=0;x+y<=n;++y)h[x+y]+=g[x]*xl[y];
			for(int x=0;x<=n;++x){g[x]=h[x];h[x]=0;f[x]+=w[i]*g[x];}
		}
		Z ans(0);
		for(int i=0;i<=n;++i)ans+=f[i]*ex[n-i];
		write(ans.x,'\n');
		return 0;
	}
}
signed main(){
	...
	for(int i=1;i<=n;++i)L[i]=i&1?inv[i]:-inv[i];
	for(int i=1;i<=k;++i){read(a[i]);w+=fpm(2,a[i]%(p-1));}
	//write(w.x,'\n');
	for(int i=2;i<=n;i+=2)f[i]=L[i]*w;
	a[0]=-1;
	exp(n+1,f,ex);
	//for(int i=0;i<=n;++i)write(ex[i].x,i==n?'\n':' ');
	//return run::main();
	if(k==1){
		return k1::main();
	}else return run::main();
	return 0;
}
